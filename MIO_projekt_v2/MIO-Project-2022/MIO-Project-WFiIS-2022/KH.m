...
NR = 1;                                  % Number if Runs
NK = 40; 						     	  % Number if Krills
MI = 80; 		                          % Maximum Iteration
...
for nr = 1:NR
    ...
    for z2 = 1:NK
        K(z2)=cost(X(:,z2), fis, x_train, y_train);
    end
    ...
    for j = 1:MI 
        ...
        Kf(j) = cost(Xf(:,j), fis, x_train, y_train);
        if 2<=j
            if Kf(j-1)<Kf(j)
                Xf(:,j) = Xf(:,j-1);
                Kf(j) = Kf(j-1);
        ...
        for i = 1:NK
            ...
            K(i)=cost(X(:,i), fis,x_train,y_train);
            if K(i)<Kib(i)
                Kib(i)=K(i);
                Xib(:,i)=X(:,i);
            end         
        ...
        if cost(X_best(:),fis,x_train,y_train) > cost(Xib(:, A),fis,x_train,y_train)
            X_best = Xib(:, A);
        end
